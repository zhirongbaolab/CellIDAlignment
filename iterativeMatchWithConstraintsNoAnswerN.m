function [violations,bestanswer,bestviolation,bestconstraint]=iterativeMatchWithConstraintsNoAnswerN(constraintnames,constraintN,matches1to2,names_2,pos_1,pos_2,iterations,constraints,...
    confidentpos1,confidentnames2)
if (exist('confidentpos1','var'))
dataDist=distFast([pos_1;confidentpos1],[pos_1;confidentpos1]);
[dall,indall]=sort(dataDist,'ascend');
grapha=GabrielGraph([pos_1;confidentpos1],indall);
else

dataDist=distFast(pos_1,pos_1);
[dall,indall]=sort(dataDist,'ascend');
grapha=GabrielGraph(pos_1,indall);
end
if (exist('confidentpos1','var'))
