...
model0 = model;
%in my variant version sigma is a vector that needs to be normalized by
%scale also
percellsigma=config.percellsigma./scale;
...
switch lower(config.motion)
    ...
    case 'grbf'
        ...
        %note this doesnt even seem to be used currently
        ...
        switch lower(config.motion)
            case 'tps'
             ...
             %   P=P./repmat(s,1,m); % normalization such that each row sums to 1
                  P=P./repmat(s,1,size(P,2)); % normalization such that each row sums to 1
...
% E-step: Given model&scene, update posterior probability matrix P.
%this would seem to be the major place where need to update to do full cov model by computing mvnpdf for each entry then doing log and norm        
...
if outliers
    %the alternative background distribution, only used for explicit out
    %liers? thats not how I read the paper
